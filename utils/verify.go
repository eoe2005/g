...
var (
	...
	_regNickName = regexp.MatchString("^[a-z0-9_-]{3,16}$")
	_regPassword = regexp.MatchString("^[a-z0-9_-]{6,18}$")
	_regHex = regexp.MatchString("^#?([a-f0-9]{6}|[a-f0-9]{3})$")
	_regUrl = regexp.MatchString("^(https?:\\/\\/)?([\\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$")
	_regHtml = regexp.MatchString("^<([a-z]+)([^<]+)*(?:>(.*)<\/\1>|\s+\/>)$")
	_regHanZi = regexp.MatchString("^[\u2E80-\u9FFF]+$")
	_regIP = regexp.MatchString("^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$")
	// _regNickName = regexp.MatchString("")
	// _regNickName = regexp.MatchString("")
	// _regNickName = regexp.MatchString("")
)
//是否是IP
